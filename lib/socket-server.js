...
module.exports = (io) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('document-update', (payload = {}) => {
            try {
                const { documentId, changes } = payload;
                const eventType = payload.eventType === 'DOCUMENT_STRUCTURE'
                    ? 'DOCUMENT_STRUCTURE'
                    : 'DOCUMENT_UPDATE';
                const normalizedDocumentVersion =
                    typeof payload.documentVersion === 'number'
                        ? payload.documentVersion
                        : typeof payload.version === 'number'
                            ? payload.version
                            : undefined;
                const documentVersion = eventType === 'DOCUMENT_UPDATE'
